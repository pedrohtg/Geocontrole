...
def cropImg(data, point, outputfile, size):
    patch = np.zeros((size, size), dtype=np.dtype('uint8')) # Ignora a ultima banda (Infravermelho)
    ...
    for l in range(lMin, lMax):
        for c in range(cMin, cMax):
            patch[l - lMin][c - cMin] = data[l][c]*255
...
def createMask(data, imgfile, outputfolder, references, geotransform, size):
    ...
    for num, ref in enumerate(references):
        outputfileimg = os.path.join(outputfolder,'Masks', imgfilesplit[0] + '_crop' + str(num) + '_mask.jpg')
...
def createData(imgfile, outputfolder, references, geotransform, size):
    ...
    if not os.path.exists(os.path.join(outputfolder,'Annotations')):
        os.mkdir(os.path.join(outputfolder,'Annotations'))
...
def createDetectionData(shapefile, shapelabels, railwayshapefile, imglist, outputfolder, size):
    ...
    for num, file in enumerate(imgs):
        ...
        createMask(groundTruth, file, outputfolder, references, geot, size)
...
def main():
    list_params = ['shapefilePath', 'shapefileLabels', 'railwayShp', 'outsize', 
                   'output_path(for images, xml)', 'folder list',
                   ]
    ...
    for i,a in enumerate(folderlist):
        ...
        if not os.path.exists(os.path.join(outputpath,app,'DetectionDataset')):
            os.mkdir(os.path.join(outputpath,app,'DetectionDataset'))
        createDetectionData(shapefilepath, shapefilelabels, railwayshape, imglist, os.path.join(outputpath,app,'DetectionDataset'), size)
