...
def saveXML(outputPath, imgName, width, height, listRect, offsetX, offsetY):
    ...
    for it, rect in enumerate(listRect):
         f.write("  <object> \n\
            ...
            </object> \n".format(int(rect[0]-offsetX+1), int(rect[1]-offsetY+1), int(rect[2]-offsetX+1), int(rect[3]-offsetY+1))
                )
...
def enlargeRailway(rlwshp, outshapename, alpha=5.1, spr=31984):
    '''
    ...
    outLayer = outDataSource.CreateLayer("railway track polygons", srs, geomType=ogr.wkbPolygon)
...
def extractRailwaySegmentsFromShapefile(layerRailway, transformRailwayImg=None, ext=None):
    ...
    for feature in layerRailway:
        ...
        if ext is not None:
            insterset = ext.Intersect(railway)
        else:
            intersect = True
        if intersect:
...
def createPatches(img, geotImg, x, y, outputWindowSize, shapefiles, imgName, datasetPath, isTrain=False, createMask=False, cropCount=0):
    ...
    centralX, centralY = coordinateToPixel(geotImg, (x, y))
    ...
    xcount = int(abs(xFinal - xBegin) / pixelWidth)
    ycount = int(abs(yFinal - yBegin) / pixelWidth)
...
def segmentPortions(segment, geoTransform, cropSize):
    ...
    numPoints = d//(cropSize/2)
    if numPoints == 0:
        return np.array([1.0])
    else:
        return np.arange(0, 1.01, 1.0/float(numPoints))

def createDatasetFromImg(imgName, railwayShapefile, featuresShapefile, cropSize, process, outputPath, task, uniqueSegments, trainFids, valFids, trainFile=None, valFile=None, valInfosFile=None):
    # Open image
    ...
    shapeZeroLayer = shapeZero.GetLayer
    ...
    # Image extent
    imgExt = utils.GetExtentGeometry(geoTransform, xAxis, yAxis).FlattenTo2D()
    ...
    for segment in segments:
        ...
        x = 0 
        y = 0
        for t in segmentPortions(segment, geoTransform, cropSize):
            x = (p2[0] - p1[0]) * t + p1[0]
            y = (p2[1] - p1[1]) * t + p1[1]
            ...
            if (yPixel - cropSize/2 >= 0 and yPixel + cropSize/2 <= yAxis and
                ...
                # If it's for test we only need the crop from image
                if process == 'test':
                    # Save crop
                    scipy.misc.imsave(os.path.join(outputPath, 'JPEGImages', name + "_crop" + str(cropCount) + '.jpg'), crop)
                ...
                if process == 'train':
                    ...
                    if fids and task == 'detection':
                        save_xml(os.path.join(outputPath, 'Annotations'),
                            name + "_crop" + str(cropCount),
                            yAxis, xAxis, interPs,
                    ...
                    if np.any(np.isin(fids, trainFids)):
                        if task == 'segmentation':
                            # Save crop
                            scipy.misc.imsave(os.path.join(outputPath, 'Train', 'JPEGImages', name + "_crop" + str(cropCount) + '.jpg'), crop)
                            # Save crop mask
                            scipy.misc.imsave(os.path.join(outputPath, 'Train', 'Masks', name + "_crop" + str(cropCount) + '_mask.jpg'), cropMask)
                        
                        if task == 'detection':
                            # Save crop
                            scipy.misc.imsave(os.path.join(outputPath, 'JPEGImages', name + "_crop" + str(cropCount) + '.jpg'), crop)
                            # Write in file that this crop it's for train
                            trainFile.write(os.path.join(outputPath, 'JPEGImages', name + "_crop" + str(cropCount)) + '\n')
                    else:
                        if task == 'segmentation':
                            # Save crop
                            scipy.misc.imsave(os.path.join(outputPath, 'Validation', 'JPEGImages', name + "_crop" + str(cropCount) + '.jpg'), crop)
                            # Save crop mask
                            scipy.misc.imsave(os.path.join(outputPath, 'Validation', 'Masks', name + "_crop" + str(cropCount) + '_mask.jpg'), cropMask)
...
def main():
    list_params = ['Images path[separeted by comma]', 'Railway shapefile', 
    ...
    for f in fs:
        if not os.path.exists(f):
            os.mkdir(f)
        for sub in subf:
            if not os.path.exists(os.join(f, sub)):
                os.mkdir(os.join(f, sub))
            if sub == 'ImageSets':
                if os.path.exists(os.join(f, sub, 'Segmentation')):
                    os.mkdir(f, sub, 'Segmentation')
    ...
    trainFidsFile = None
    if task == 'detection':
        valInfoFidsFile = open(os.path.join(outRoot, 'ImageSets', 'Segmentation', 'bridge_test.txt'), 'w')
        valFidsFile = open(os.path.join(outRoot, 'ImageSets', 'Segmentation', 'test.txt'), 'w')
        trainFidsFile = open(os.path.join(outRoot, 'ImageSets', 'Segmentation', 'trainval.txt'), 'w')
